...
global data_folder "/Users/mac/Documents/Labor/bracero" // INSERT PATH
...
** added - drop if there are no workers on the farm
drop if TotalHiredSeasonal_final == 0
* For graphs
...
** added
replace Jamaican_final = 0 if Jamaican_final==.
replace Bahamian_final = 0 if Bahamian_final==. 
replace Canadian_final = 0 if Canadian_final==. 
replace PuertoRican_final = 0 if PuertoRican_final==. 
replace BWIOthers_final = 0 if BWIOthers_final==. 
replace OtherForeign_final = 0 if OtherForeign_final==.
...
* FIGURE 5-replication See wage variation over the year - take 1955 as the authors use it
preserve
keep if Year==1955 & State=="TX"
egen average_frac = mean(mex_frac), by(Year)
collapse (mean) mex_frac average_frac Mexican, by (Month)
line average_frac Month || line mex_frac Month, ytitle("") legend(label(1 "Average over the year") label(2 "Fraction of braceros"))
graph save mexfrac_1955.gph, replace
graph export "../Replication_figures/Figure5_mexfrac_1955.pdf", replace
graph export mexfrac_1955.pdf, replace
restore

* Diffs in Diffs regressions

** added - average from 1954 to 1962
preserve

* First generate treatment intensity variable, denominator is hired seasonal workers
keep if Year>=1954 & Year<=1962 
gen mex_frac_avg = Mexican/(Mexican+NonMexican) 
gen mex_num_avg = Mexican
collapse (mean) mex_frac_avg, by(State)
** added - APPENDIX TABLE A2

	************** export table of mex_frac_avg values
	gsort -mex_frac_avg
	mkmat mex_frac_avg, matrix(A) rownames(State)
	mat list A
	outtable using tables_bracero, mat(A) replace caption("APPENDIX TABLE A2a: Mex fraction 1954-1962 average") format(%04.3f )
	**************
...
gen post = Year>=1965
gen treatment_frac = post * mex_frac_avg

gen post_2 = Year>=1962
gen treatment_frac_2 = post_2 * mex_frac_avg


preserve
* Now generate denominator for scaling: total hired farm workers in average month of 1957-1962
* Note that total farmworkers series only begins in 1957
keep if Year>=1957 & Year<=1962
gen farm_tot_avg = Farmworkers_Hired*1000
collapse (mean) farm_tot_avg, by(State)
...
gen none = mex_frac_avg == 0
gen low = (mex_frac_avg >0 & mex_frac_avg <0.1)
gen high = mex_frac_avg >=0.1 & mex_frac_avg <.     // This is: AR, AZ, CA, NM, SD, TX, CO, NE
...
** TABLE 1 - take similar states - region is West South central
drop if Region != 7
...
esttab using tables_bracero_limited_treatment.tex, se ar2 nostar compress append keep(treatment_frac) ///
	booktabs alignment(D{.}{.}{-1}) title(TABLE 1: Differences-in-differences with continuous treatment - Region West South Central, quarterly)  ///
	scalars(N_clust) 
	
esttab using "../Replication_figures/tables_bracero.tex", se ar2 nostar compress append keep(treatment_frac) ///
	booktabs alignment(D{.}{.}{-1}) title(TABLE 8: Differences-in-differences with continuous treatment - Region West South Central, quarterly)  ///
	scalars(N_clust) 
...
esttab using tables_bracero_limited_treatment.tex, se ar2 nostar compress append keep(treatment_frac) ///
	booktabs alignment(D{.}{.}{-1}) title(TABLE 1: Semielasticities, DD with continuous treatment, Region West South Central, quarterly)  ///
	scalars(N_clust) 

mat pvals = pvals[2...,1...]	// Drop leading blank
mat pvals = pvals'			// transpose for LaTeX conversion
outtable using "tables_bracero_limited_treatment", mat(pvals) append norowlab nobox caption("TABLE 1: p vals of semielasticities") format(%5.4f %5.4f %5.4f %5.4f)
restore
...
graph twoway lpoly realwage_daily time_q if none & Year>=1945, degree(1) bwidth(2) clcolor($color_control) clwidth(thin) clpattern(vshortdash)  ///
	...
	legend(colfirst order (- "{it:Bracero} fraction ({it:B}/{it:L}) in 1954-1962:" - - 1 2 3) cols(2) region(lstyle(none) margin(none)) ///
	label(1 "No exposure ({it:B}/{it:L} = 0)") label(2 "Low exposure (0 < {it:B}/{it:L} < 0.1)") label(3 "High exposure ({it:B}/{it:L} {&ge} 0.1)") size(small)) 
...
esttab using "../Replication_figures/tables_bracero.tex", se ar2 nostar compress append keep(treatment_frac) ///
	booktabs alignment(D{.}{.}{-1}) title(TABLE 3: Differences-in-differences with continuous treatment, quarterly)  ///
	scalars(N_clust) 
...
esttab using "../Replication_figures/tables_bracero.tex", se ar2 nostar compress append keep(treatment_frac) ///
	booktabs alignment(D{.}{.}{-1}) title(TABLE 4: Semielasticities, DD with continuous treatment, quarterly)  ///
	scalars(N_clust) 
...
outtable using "../Replication_figures/tables_bracero", mat(pvals) append norowlab nobox caption("TABLE 5: p vals of semielasticities") format(%5.4f %5.4f %5.4f %5.4f)
...
esttab using "../Replication_figures/tables_bracero.tex", se ar2 nostar compress append keep(treatment_frac) ///
	booktabs alignment(D{.}{.}{-1}) title(TABLE 6: Differences-in-differences with continuous treatment, monthly, Jan 1954-Jul 1973 only)  ///
	scalars(N_clust ) 
...
*	keep if mex_frac_55>0						// only exposed states
	keep if Month==`m'
	collapse (mean) ln_domestic_seasonal mex_frac_avg treatment_frac Month, by(State_FIPS Year)
	tsset State_FIPS Year
	xtreg ln_domestic_seasonal ib1964.Year##c.mex_frac_avg, fe vce(cluster State_FIPS)
...
graph twoway rspike lower_bound upper_bound time_m_es if mon_es==6, lcolor(gs10) lwidth(*.5) ///
	...
	ytitle("{it:coeff} (Year `=uchar(0215)' 1954-1962 average {it:bracero} stock)", margin(medium)) xtitle("Year", margin(small))  ///
...
foreach m of numlist 6 {
	preserve
	keep if Year >=1954 & Year<=1973  			// keep only cells with non-missing wages
	keep if mex_frac_avg>0						// only exposed states
	keep if Month==`m'
	collapse (mean) ln_domestic_seasonal mex_frac_avg treatment_frac Month, by(State_FIPS Year)
	tsset State_FIPS Year
	xtreg ln_domestic_seasonal ib1964.Year##c.mex_frac_avg, fe vce(cluster State_FIPS)
...
graph twoway rspike lower_bound upper_bound time_m_es if mon_es==6, lcolor(gs10) lwidth(*.5) ///
	...
	scheme(s1color) xline(26, lcolor(black) lpattern(vshortdash) lwidth(thin)) ///
	xline(59, lcolor(black) lpattern(vshortdash) lwidth(thin)) plotregion(lcolor(none)) yline(0, lcolor(gs14) lwidth(vthin)) ///
	ytitle("{it:coeff} (Year `=uchar(0215)' 1954-1962 {it:bracero} stock)", margin(medium)) xtitle("Year", margin(small))  ///
	xlabel(-60(60)150,format(%tmCY)) xmtick(-72(12)162) ylabel(,format(%03.1f)) ///
	legend( order(- "Month:" 2 4) rows(1) region(lstyle(none) margin(none)) ///
	label(2 "June") label(4 "September") size(small) span)
...
foreach x in "New Mexico" "Arizona" "California" "Texas" "South Dakota" "Nebraska" "Colorado" "Arkansas" "Nevada" {
...
grc1leg "dom_mex_raw_New Mexico.gph" "dom_mex_raw_Arizona.gph" "dom_mex_raw_California.gph" ///
	"dom_mex_raw_Texas.gph" "dom_mex_raw_South Dakota.gph" "dom_mex_raw_Nebraska.gph"   ///
	"dom_mex_raw_Colorado.gph" "dom_mex_raw_Arkansas.gph" "dom_mex_raw_Nevada.gph", ///
...
erase "dom_mex_raw_Colorado.gph" 
erase "dom_mex_raw_Arkansas.gph" 
erase "dom_mex_raw_South Dakota.gph"

** Plot foreigners other than mexican over time, and mexican with highest exposure
foreach x in "New Mexico" "Arizona" "California" "Texas" "South Dakota" "Nebraska" "Colorado" "Arkansas" "Nevada" {

	line ln_foreign ln_Mexican time_m if State_name=="`x'" & time_m > ym(1954,1) & time_m < ym(1973,3), cmissing(n n) ///
		lcolor(black gs12) scheme(s1color) xline(26, lcolor(black) lpattern(vshortdash) lwidth(thin)) ///
		xline(59, lcolor(black) lpattern(vshortdash) lwidth(thin)) plotregion(lcolor(none)) ///
		ytitle("{it:ln} Seasonal workers", margin(small) size(large)) ylabel(#4, format(%3.0f) labsize(large)) ///
		xtitle("Year", margin(small) size(large)) xlabel(-60(60)150,format(%tmCY) labsize(large)) xmtick(-72(12)162) aspect(.5) ///
		legend(order(1 2) rows(1) region(lstyle(none) margin(none)) ///
		label(1 "Foreign") label(2 "Mexican") size(vsmall) span) title(`"`x'"', margin(medsmall))
	graph save "for_mex_raw_`x'.gph", replace
	*graph export "dom_mex_raw_`x'.pdf", replace

}
	
grc1leg "for_mex_raw_New Mexico.gph" "for_mex_raw_Arizona.gph" "for_mex_raw_California.gph" ///
	"for_mex_raw_Texas.gph" "for_mex_raw_South Dakota.gph" "for_mex_raw_Nebraska.gph"   ///
	"for_mex_raw_Colorado.gph" "for_mex_raw_Arkansas.gph" "for_mex_raw_Nevada.gph", ///
	rows(3) scale(1.2) xsize(3.8) fysize(120) scheme(s1color) altshrink name(resized, replace)
graph export dom_mex_raw_combined.pdf, replace

erase "for_mex_raw_New Mexico.gph"
erase "for_mex_raw_Nebraska.gph" 
erase "for_mex_raw_Arizona.gph"
erase "for_mex_raw_Texas.gph"
erase "for_mex_raw_California.gph"
erase "for_mex_raw_Nevada.gph"
erase "for_mex_raw_Colorado.gph" 
erase "for_mex_raw_Arkansas.gph" 
erase "for_mex_raw_South Dakota.gph"
...
esttab using "../Replication_figures/tables_bracero.tex", se ar2 nostar compress append  keep(treatment_frac) ///
	booktabs alignment(D{.}{.}{-1}) title(TABLE 7: Differences-in-differences with continuous treatment, monthly)  ///
	scalars(N_clust ) 
...
graph twoway line season_realwage_hourly_none time_h if Year>=1948 & Year<=1972, c(stairstep) clcolor($color_control) clwidth(thin) clpattern(shortdash) ///
	...
	label(1 "No exposure ({it:B}/{it:L} = 0)") label(2 "Low exposure (0 < {it:B}/{it:L} < 0.1)") label(3 "High exposure ({it:B}/{it:L} {&ge} 0.1)")  ///
...
foreach q of numlist 1/2 {
	preserve
	*collapse (mean) realwage_hourly mex_frac_avg treatment_frac Year quarter, by(State_FIPS time_q)
	keep if Year >=1948 & Year<=1971  			// keep only cells with non-missing wages
	keep if quarter==`q'
	collapse (mean) realwage_hourly mex_frac_avg treatment_frac quarter, by(State_FIPS Year)
	tsset State_FIPS Year
	xtreg realwage_hourly ib1964.Year##c.mex_frac_avg, fe vce(cluster State_FIPS)
	matrix coeffs = r(table)
	matrix coeffs = coeffs'
...
graph twoway rspike lower_bound upper_bound time_q_es if qtr_es==1, lcolor(gs10) lwidth(*.5) ///
	...
	ytitle("{it:coeff} (Year `=uchar(0215)' avg 1954-1962 {it:bracero} stock)", margin(medium)) xtitle("Year", margin(small))  ///
	xlabel(-60(20)60,format(%tqCY)) xmtick(-60(4)60) ylabel(,format(%03.1f)) ///
...
graph export "../Replication_figures/Figure3_Replication_nonmex.pdf", replace
...
gen all_farm_tot = (Farmworkers_Hired*1000)/farm_tot_avg
gen mex_tot = Mexican/farm_tot_avg
...
* Technology adoption - comapare similar states - AR and OK
preserve
keep if State == "AR" | State == "OK"
collapse (mean) Cotton_machine Sugarbeet_machine, by(State_FIPS Year)
keep if Year>1950
...
line Cotton_machine Year if State_FIPS==5 || line Cotton_machine Year if State_FIPS==40,ytitle("") legend(label(1 "Arkansas") label(2 "Oklahoma")) xline(1965)

graph save cotton_AR_OK.gph, replace
graph export "../Replication_figures/Figure8_Replication_cotton_AR_OK.pdf", replace
graph export cotton_AR_OK.pdf, replace

restore

preserve
collapse (mean) Cotton_machine Sugarbeet_machine Sugarbeet_monogerm ln_Mexican ln_NonMexican mex_frac_avg treatment_frac high low none, by(State_FIPS Year)
keep if Year>1950
...
** added - production of tomatoes in CA and OH
** filter by state to get CA and OH production
preserve
keep if State=="CA" | State=="OH"
keep if Year>=1955 & Year<=1970 // data only available for these years
collapse (mean) Tomatoes_total, by(State_FIPS Year)
reshape wide Tomatoes_total, i(Year) j(State)
gen tomatoes_ratio = Tomatoes_total6/Tomatoes_total39


graph bar tomatoes_ratio, over(Year) ytitle("Ratio of CA/OH production")
graph export "../Replication_figures/Figure6_Tomatoes_CA_OH.pdf", replace
graph export Tomatoes_CA_OH.pdf, replace
restore

preserve
collapse (mean) SugarBeets Cotton Tomatoes_total Lettuce Strawberries_total Citrus Cantaloupes BrusselsSprouts Asparagus_total Celery Cucumbers_pickle Tomatoes_fresh Tomatoes_proc Strawberries_fresh Strawberries_proc Asparagus_fresh Asparagus_proc ln_Mexican ln_NonMexican mex_frac_avg treatment_frac high low none, by(State_FIPS Year)
tsset State_FIPS Year
...
xi i.Year*mex_frac_avg 		// interaction term for event study regressions
...
graph export crops_beets_strawberries.pdf, replace
...
graph twoway line guestwork year, lcolor(black) c(stairstep) yaxis(1) ylabel(0(500000)1000000, format(%9.0fc) axis(1)) ///
	ytitle("", margin(medium) axis(1)) ///
	|| line apprehensions year, lcolor(black) c(stairstep) lpattern(shortdash) yaxis(1) ylabel(#3, format(%9.0fc) axis(1)) /// 
	xtitle("Year", margin(medium)) plotregion(lcolor(none)) scheme(s1color) aspect(.6) ///
	legend(label (1 "{it:Bracero} arrivals, US total") label(2 "Apprehensions") cols(1) region(lcolor(none))) ///
	xline(1964.85, lcolor(black) lpattern(dot) lwidth(thin)) xline(1962.3, lcolor(black) lpattern(dot) lwidth(thin))
	
graph export "../Replication_figures/Figure4_Replication_braceros_apprehensions.pdf", replace 
...
graph twoway line braceros Year if State=="CA", yaxis(1) lcolor(black) c(stairstep)  /// 
	...
	xline(1964.85, lcolor(black) lpattern(dot) lwidth(thin)) xline(1962.3, lcolor(black) lpattern(dot) lwidth(thin))
graph export "../Replication_figures/Figure7_bracero_tomato_ca.pdf", replace
...
graph twoway line braceros Year if State=="OH", yaxis(1) lcolor(black) c(stairstep)  /// 
	...
	legend(label (1 "Peak annual {it:bracero} stock (left axis)") label(2 "Tomato harvest mechnization (right axis)") cols(1) region(lcolor(none))) aspect(.5)  ///
	xline(1964.85, lcolor(black) lpattern(dot) lwidth(thin)) xline(1962.3, lcolor(black) lpattern(dot) lwidth(thin))
graph export "../Replication_figures/Figure7_bracero_tomato_oh.pdf", replace
